...
int get_png_data_IHDR(struct data_IHDR *out, FILE *fp, long offset, int whence){
  fseek(fp, offset, whence);   //Traverse the file pointer by the offset in the file location
  
}

int is_png(U8 *buf, size_t n){
  U8 png_magic_number[8] = {0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A}; // This is the magical number, see lab manual
  int i = 0; // while loop counter
  if(n < 8){ // if the number of bytes is smaller than 8, then it is not valid
    return 0; // should print in main()
  }
  
  while(i < 8){
    if(buf[i] != png_magic_number[i]){
      return 0;
    }else{
  ...
  return 1; // the magic number matches the buf points to, so we confirm it is a PNG file
}
